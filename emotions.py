...
ap.add_argument("--image", help="image path", default=None)
mode = ap.parse_args().mode
image_path = ap.parse_args().image
...
# emotions will be displayed on your face from the webcam feed
    ...
    # Define padding size
    top, bottom, left, right = [100, 50, 50, 100]
    ...
    if image_path is not None:
        ...
        for (x, y, w, h) in faces:
            cv2.rectangle(frame_with_border, (x, y-50), (x+w, y+h+10), (255, 0, 0), 2)
            ...
            roi = cv2.resize(frame_with_border[y:y+h,x:x+w], (227, 227))
            ...
            cv2.putText(frame_with_border, f"{gender}, {age}, {emotion_dict[maxindex]}", (x+20, y-60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)

        cv2.imshow('Image', cv2.resize(frame_with_border,(700, 500),interpolation = cv2.INTER_CUBIC))
        cv2.waitKey(0)
